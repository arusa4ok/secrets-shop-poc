...
async function main() {
  ...
  const csvPath = path.resolve(args.csv ?? path.join(__dirname, "../tmp/medusa-only-products.csv"))
  ...
  const logPath = path.join(outputDir, "cleanup-medusa-only-log.ndjson")
  const failuresPath = path.join(outputDir, "cleanup-medusa-only-failures.ndjson")
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    for (const row of batch) {
      ...
      try {
        const updatedProduct = await fetchWithRetry(`${backendUrl}/admin/products/${productId}`, {
          method: "POST",
          headers: authHeaders,
          body: JSON.stringify({ status: "draft" }),
        })
        const entry = { type: "updated", productId, handle, status: updatedProduct.product.status }
        updated.push(entry)
        await appendLog(logPath, entry)
        console.log(`Drafted product ${handle} (${productId})`)
      } catch (err) {
        const entry = { type: "update_error", productId, handle, reason: err.message }
        failed.push(entry)
        await appendLog(failuresPath, entry)
        console.error(`Failed to draft ${handle}: ${err.message}`)
      }
  ...
  const summaryPath = path.join(outputDir, "cleanup-medusa-only-summary.json")
